...
g.wait_instance = g.wait_instance or function(parent, resolver)
    local inst = resolver()
    if inst then return inst end
    local conn
    conn = parent.ChildAdded:Connect(function() inst = resolver() end)
    while not inst do hb() end
    if conn then conn:Disconnect() end
    return inst
end

g.return_char = g.return_char or function(Player, timeout)
...
getgenv().booth_folder_find_for_mic_up = function()
    ...
    if cache and cache:IsA("Folder") and cache:IsDescendantOf(game or workspace) then return cache end
...
wait(0.1)
if not getgenv().booth_folder_found_mic_up then pcall(function() getgenv().booth_folder_find_for_mic_up() end) end
...
getgenv().get_character_main_local = function(player)
    ...
    if getgenv().notify and typeof(getgenv().notify) == "function" then getgenv().notify("Error", "Player may have left, no Character was ever found.", 5) end

    return nil
end

getgenv().getRoot = getgenv().getRoot or function(char)
    local rootPart = getgenv().HumanoidRootPart or getgenv().Character and getgenv().Character:FindFirstChild("HumanoidRootPart") or char and char:FindFirstChild('HumanoidRootPart') or char:FindFirstChild('Torso') or char:FindFirstChild('UpperTorso')
    return rootPart
end
...
wait(0.1)
getgenv().Script_Version = "10.5.5-FlamesHub"
...
if SCRIPT_EXECUTED or getgenv().SCRIPT_EXECUTED and not _G.SCRIPT_EXECUTED == true then  
    return Notification:External_Notification("Warning", "Flames Hub is already running!", 5) -- this notification library is fine in this format since it's not the typical one with the special princess sissy preset arguments.
end
...
if enabled_vc == true then -- This just retrieves that response you get when you called "IsVoiceEnabledForUserIdAsync(game.Players.LocalPlayer.UserId)", even though I don't know why I have to match it to a Boolean, but ok.
    Notification:External_Notification("Success", "Connected with VC successfully!", 3)
elseif (enabled_vc == false and (not game.PlaceId == 6884319169)) or (not game.PlaceId == 15546218972) then
    Notification:External_Notification("Success", "Connected.", 5) -- I'm never sure why I did this tbh, just kind of figured it'd separate the messages, but never really mattered.
elseif enabled_vc == false and game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    Notification:External_Notification("Warning", "You do not have VC, you may be kicked!", 5) -- Alerting that since the user does not have Voice Chat, MIC UP may remove them from the game (since it's literally a VC game lmao).
else
    Notification:External_Notification("Info", "You do not have VC and you're not in MIC UP, it's alright.", 5) -- I actually have no idea when or why I added this, but I guess I kept it, so it might be necessary, I don't know man.
end
wait()
...
if TextChannels and RBXGeneral then
    pcall(function()
        ...
        wait(1.8)
        RBXGeneral:DisplaySystemMessage(tostring(g.Script_Version))
        wait(1.5)
...
for attempt = 1, MAX_ATTEMPTS do
    local success, result = pcall(function()
        return loadstring(game:HttpGet("https://codeberg.org/talkinboutlol/Flames_Hub/raw/branch/main/UIs/Rayfield.lua"))()
    end)
...
if executor_Name == "Solara" or executor_Name == "Sonar" or executor_Name == "Xeno" or executor_Name == "Macsploit" then
    Window = Rayfield:CreateWindow({
        Name = "📍 Flames Hub 📍 | "..tostring(g.Script_Version).." | "..tostring(executor_Name),
...
if not getgenv().AbsoluteAntiLagFlamesHub then
    ...
    for _, child in pairs(Workspace:GetDescendants()) do optimize(child) end
    wait(0.1)
...
Callback = function(turnOnRGBPart)
    if turnOnRGBPart then
        ...
        local function apply_tween_color(part, new_color)
            local tweenInfo = TweenInfo.new(getgenv().RainbowSpeed, Enum.EasingStyle.Linear, Enum.EasingDirection.Out)
            local tween = TweenService:Create(part, tweenInfo, { Color = new_color })
            tween:Play()
        end
...
Callback = function(anims_spoofed)
    ...
    if anims_spoofed then
        ...
        getgenv().custom_anim_conn = RunService.Heartbeat:Connect(function()
...
getgenv().gameplay_paused_toggle = function(state)
    ...
    pcall(function() GuiService:SetGameplayPausedNotificationEnabled(state) end)
end
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    ...
    Callback = function(TypeEffectStall)
        local Booth_Remote = getgenv().ReplicatedStorage:FindFirstChild("UpdateBoothText", true)
        if Booth_Remote and Booth_Remote:IsA("RemoteEvent") then
            local function typeText(remote, text, typing_speed)
                local currentText = ""
                for i = 1, #text do
            ...
            local text_to_type = tostring(TypeEffectStall)
            local typing_speed = 1.1
            typeText(Booth_Remote, text_to_type, typing_speed)
        end
    end,})
else
...
Callback = function(bangPlayerTarget)
    ...
    function bang_plr_bypass(target)
        ...
        if not target then
            ...
            return getgenv().notify("Error", "Player was not found!", 5)
        end

        if not target.Character then
            getgenv().unload_bang_function()
            return getgenv().notify("Error", "Target's Character was not found!", 5)
        end
...
Callback = function(get_spin_speed)
    ...
    if not spinning_velocity then
        ...
        spinning_velocity.Parent = HumanoidRootPart
...
Callback = function()
    ...
    local HumanoidRootPart = getgenv().HumanoidRootPart or get_root(LocalPlayer, 10)
    if not HumanoidRootPart then return end
...
if not getgenv().Character:FindFirstChild("Animate", true) then
    ...
    function reset_walk_while_emoting()
        ...
        wait(0.1)
        for _, anim_track in pairs(getgenv().Humanoid:GetPlayingAnimationTracks()) do
            anim_track:Stop()
        end
    end
    wait(0.2)
...
Callback = function(michaelJackson)
    ...
    if michaelJackson then
        ...
        task.wait(0.1)
        if humanoid then
            humanoid:PlayEmoteAndGetanim_trackById(4391208058)
        end
        task.wait(0.2)
        ...
        if root then root.Anchored = true end
        task.wait()
        if animate and animate:IsA("LocalScript") and animate:IsDescendantOf(game) then animate.Disabled = false end
        task.wait(0.2)
        if humanoid and humanoid.Parent and humanoid:IsDescendantOf(game) then humanoid:ChangeState(3) end
        task.wait(0.2)
        if humanoid and humanoid.Parent and humanoid:IsDescendantOf(game) then humanoid.WalkSpeed = 16 end
        task.wait(0.2)
        if root and root.Parent and root:IsDescendantOf(game) then root.Anchored = false end
    end
end,})
...
if game.PlaceId == 17274762379 then
    ...
    Callback = function()
        ...
        local RemoteFunctions = Remotes or Remotes:FindFirstChild("RemoteFunctions")
        local Codes = RemoteFunctions:FindFirstChild("Codes")
        if Codes and Codes.Parent and Codes:IsA("RemoteFunction") then Codes:InvokeServer("3mvisits") end
    end,})
    ...
    Callback = function(player_entered)
        ...
        if not found_this_plr then return getgenv().notify("Error", "Player was not found or does not exist.", 5) end
    ...
    if require and typeof(require) == "function" then
        ...
        Callback = function(text_to_display)
            ...
            if executor_Name == "Solara" or string.find(executor_Name, "JJSploit") or executor_Name == "Xeno" then
                return getgenv().notify("Error", "This feature isn't supported on this executor.", 5)
            end
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    ...
    Callback = function(getThisPlayer)
        ...
        local bruh_user = findplr(getThisPlayer)

        if not bruh_user then
        ...
        addPlayerToBoothTable(bruh_user)
    end,})
    ...
    Callback = function(RemoveBoothWhitelist)
        ...
        local dawg_user = findplr(RemoveBoothWhitelist)
        ...
        removePlayerFromBoothTable(dawg_user)
    end,})
else
...
Callback = function(addThatUserHere)
    ...
    local bruh_user = findplr(addThatUserHere)
    ...
    addPlayerToScriptWhitelistTable(bruh_user)
end,})
...
Callback = function(removeThatUserOverHere)
    ...
    local bruh_user = findplr(removeThatUserOverHere)
    ...
    removePlayerFromScriptWhitelistTable(bruh_user)
end,})
...
getgenv().AddPlayerToGlobalTable = function(player)
    if next(getgenv().SinglePlayerTable) then
        ...
        elseif not Players[player.Name] then
            return getgenv().notify("Error", tostring(player)..", does not exist!", 5)
        elseif bruh_user == getgenv().LocalPlayer.Name then
            return getgenv().notify("Error", "You cannot add yourself!", 5)
        end
    end
end
...
getgenv().copy_animations_from_whitelisted_player = function()
    for _, anim_track in pairs(Humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end
    for _, anim_track in pairs(their_humanoid:GetPlayingAnimationTracks()) do
        if not string.find(anim_track.Animation.AnimationId, "507768375") then
            local copied_anim = Humanoid:LoadAnimation(anim_track.Animation)
            copied_anim:Play(0.1, 1, anim_track.Speed)
            copied_anim.TimePosition = anim_track.TimePosition

            task.spawn(function()
                anim_track.Stopped:Wait()
                copied_anim:Stop()
                copied_anim:Destroy()
            end)
        end
...
PlaceholderText = "User",
RemoveTextAfterFocusLost = true,
Callback = function(this_user_get)
    local bruh_user = findplr(this_user_get)
    if not bruh_user then return g.notify("Error", "Player not found or does not exist.", 3) end
    AddPlayerToGlobalTable(bruh_user)
end,})
...
Callback = function(CopyAnimPlr)
    local dawg_user = findplr(CopyAnimPlr)
    if not dawg_user then return g.notify("Error", "Player not found or does not exist.", 3) end
    RemovePlayerFromGlobalTable(dawg_user)
end,})

getgenv().CopyAnimButtonPlr = Tab14:CreateButton({
Name = "CopyAnim Whitelisted Plr",
Callback = function()
    local Character = getgenv().Character or LocalPlayer.Character or g.get_char(LocalPlayer, 5)
    local Humanoid = getgenv().Humanoid or getgenv().Humanoid or g.get_human(LocalPlayer, 5)
    local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character and getgenv().Character:FindFirstChild("HumanoidRootPart") or g.get_root(LocalPlayer, 5)
    local the_player = get_that_plr()
    if not the_player then return getgenv().notify("Error", "Player not found or does not exist.", 3) end
    local TheirCharacter = the_player.Character or g.get_char(the_player, 3)
    local get_their_humanoid = TheirCharacter:FindFirstChildWhichIsA("Humanoid") or TheirCharacter:WaitForChild("Humanoid")
    for _, anim_track in pairs(Humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end
    -- This looks so ChatGPT, but I wrote this a long fucking time ago, thought I was cooking, but I really didn't.
    local get_target_character = the_player.Character or g.get_char(the_player, 3)
    local their_humanoid = get_target_character:FindFirstChildWhichIsA('Humanoid') or get_target_character:WaitForChild('Humanoid') or get_human()
...
CurrentValue = getgenv().copy_all_whitelisted_anims or false,
Flag = "DoCopyAnimLoop",
Callback = function(get_loop_copy_anim)
    if get_loop_copy_anim then
        local the_player = get_whitelisted_plr_from_tbl()
        if not the_player then return getgenv().notify("Error", "No whitelisted player(s) found in table!", 5) end
        local Character = getgenv().Character or LocalPlayer.Character or get_char(LocalPlayer, 3)
        if not Character then return getgenv().notify("Error", "Your character does not exist yet, try again when you respawn.", 6)  end
        local Humanoid = getgenv().Humanoid or getgenv().Humanoid or get_human(LocalPlayer, 3)
        local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character:FindFirstChild("HumanoidRootPart") or get_root(LocalPlayer, 3)
        local TheirCharacter = the_player.Character or get_char(the_player, 3)
        local get_their_humanoid = TheirCharacter:FindFirstChildWhichIsA("Humanoid") or get_human(the_player, 3)
        for _, anim_track in pairs(Humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end
        local their_humanoid = TheirCharacter:FindFirstChildWhichIsA('Humanoid') or TheirCharacter:WaitForChild('Humanoid')
        local default_anim_LocalScript = get_animate_localscript(Character)
        getgenv().copy_animations = function()
            if not default_anim_LocalScript or not default_anim_LocalScript:IsA("LocalScript") then
                return getgenv().notify("Error", "LocalScript: Animate does not exist or is not a LocalScript.", 3)
            else
                default_anim_LocalScript.Disabled = true
            end
        ...
        getgenv().copy_all_whitelisted_anims = true
        while getgenv().copy_all_whitelisted_anims == true do
        task.wait(.5) copy_animations() end
    else
        local the_player = get_that_plr()
        if not the_player then return g.notify("Error", "Player does not exist.", 3) end
        local Character = getgenv().Character or getgenv().LocalPlayer.Character or g.get_char(LocalPlayer, 3)
        if not Character then return g.notify("Error", "Your Character does not exist or was not found.", 3) end
        local Humanoid = getgenv().Humanoid or Character and Character:FindFirstChild("Humanoid") or g.get_human(LocalPlayer, 3)
        if not Humanoid then return g.notify("Error", "Your Humanoid does not exist or was not found.", 3) end
        local HumanoidRootPart = getgenv().HumanoidRootPart or Character and Character:FindFirstChild("HumanoidRootPart") or g.get_root(LocalPlayer, 3)
        if not HumanoidRootPart or not HumanoidRootPart.Parent then return g.notify("Error", "Your HumanoidRootPart does not exist or was not found.", 3) end
        getgenv().copy_all_whitelisted_anims = false
        local Humanoid = Character:FindFirstChildWhichIsA("Humanoid") or Character:WaitForChild("Humanoid")
        local TheirCharacter = the_player.Character or g.get_char(the_player, 3)
        local get_their_humanoid = TheirCharacter:FindFirstChildWhichIsA("Humanoid") or TheirCharacter:WaitForChild("Humanoid")
        for _, anim_track in pairs(Humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end
        local function stopAnimations() for _, anim_track in pairs(get_their_humanoid:GetPlayingAnimationTracks()) do pcall(function() anim_track:Stop() end) end end
        for i = 1, 10 do stopAnimations() end
        getgenv().reset_default_animations = function()
            local animate_script = Character and Character:FindFirstChild("Animate", true)
            if animate_script and animate_script.Parent and animate_script:IsA("LocalScript") then
        ...
        reset_default_animations()        
    end
end,})
wait(0.2)
if getgenv().copy_all_whitelisted_anims == true then
    getgenv().copy_all_whitelisted_anims = false
    if getgenv().LoopCopyTheEmotePlr then getgenv().LoopCopyTheEmotePlr:Set(false) end
end
wait(0.1)
...
Callback = function(getJerkingScript)
    if getJerkingScript then
        if getgenv().Jerking then return getgenv().notify("Error", "Jerk Off Animation already loaded.", 5) end
        wait(0.1)
        local Character = getgenv().Character or getgenv().LocalPlayer.Character or g.get_char(LocalPlayer, 3)
        if not Character then return g.notify("Error", "Your Character does not exist or was not found.", 3) end
        local Humanoid = getgenv().Humanoid or Character and Character:FindFirstChildWhichIsA("Humanoid") or g.get_human(LocalPlayer, 3)
        if not Humanoid or not Humanoid.Parent or not Humanoid:IsDescendantOf(game) then return g.notify("Error", "Your Humanoid does not exist or was not found.", 3) end
        ...
        if getgenv().JerkAnim and getgenv().Jerk then
            ...
            return getgenv().notify("Error", "Jerk off Animation is not loaded!", 5)
        end
    end
end,})
...
getgenv().set_new_materials = function(new_material)
    if not new_material or typeof(new_material) ~= "EnumItem" then return getgenv().notify("Error", "Invalid material detected, try again.", 5) end
    for _, part in ipairs(getgenv().Workspace:GetDescendants()) do
        if part:IsA("BasePart") and not find_character_skip(part) then
            if not getgenv().OriginalMaterials[part] then getgenv().OriginalMaterials[part] = part.Material end
...
getgenv().default_materials = function()
    for part, originalMaterial in pairs(getgenv().OriginalMaterials) do if part and part:IsA("BasePart") then part.Material = originalMaterial end end
    getgenv().OriginalMaterials = {}
...
getgenv().set_new_transparency = function(new_transparency)
    if type(new_transparency) ~= "number" or new_transparency < 0 or new_transparency > 1 then
        return getgenv().notify("Error", "Invalid transparency value. Must be between 0 and 1.", 5)
    end
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    getgenv().BasePlate_ColorChange = Tab18:CreateSlider({
        ...
        Increment = 0.1,
        Suffix = "",
        CurrentValue = getgenv().Flags and getgenv().Flags["MICUPTransparency"] or 0,
        Flag = "MICUPTransparency",
        Callback = function(baseplate_transparency)
            local map = getgenv().Workspace:FindFirstChild("map")
            if map and map.Parent and map:IsDescendantOf(game) then
                local important = map:FindFirstChild("Important")
                if not important or not important.Parent or not important:IsDescendantOf(game) then return g.notify("Error", "Important folder not found.", 3) end
                task.wait(0.1)
                if not important or not important.Parent or not important:IsDescendantOf(game) then return g.notify("Error", "Important folder not found.", 3) end
                local baseplate_folder = important:FindFirstChild("Baseplate")
                if not baseplate_folder or not baseplate_folder.Parent or not baseplate_folder:IsDescendantOf(game) then return g.notify("Error", "Baseplate folder not found.", 3) end
                local base_part = baseplate_folder:FindFirstChild("Baseplate")
                if not base_part or not base_part.Parent or not base_part:IsA("BasePart") then return g.notify("Error", "Baseplate not found.", 3) end
                base_part.Transparency = baseplate_transparency
                return
            end
            task.wait(0.1)
            local map_2 = getgenv().Workspace:FindFirstChild("map")
            if not map_2 or not map_2.Parent or not map_2:IsDescendantOf(game) then return g.notify("Error", "Map not found.", 3) end
            local important_2 = map_2:FindFirstChild("Important")
            if not important_2 or not important_2.Parent or not important_2:IsDescendantOf(game) then return g.notify("Error", "Important folder not found.", 3) end
            local baseplate_folder_2 = important_2:FindFirstChild("Baseplate")
            if not baseplate_folder_2 or not baseplate_folder_2.Parent or not baseplate_folder_2:IsDescendantOf(game) then return g.notify("Error", "Baseplate folder not found.", 3) end
            local base_part_2 = baseplate_folder_2:FindFirstChild("Baseplate")
            if not base_part_2 or not base_part_2.Parent or not base_part_2:IsA("BasePart") then return g.notify("Error", "Baseplate not found.", 3) end
            base_part_2.Transparency = baseplate_transparency
        end,})
    task.wait(0.2)
    local init_map = getgenv().Workspace:FindFirstChild("map")
    if init_map and init_map.Parent and init_map:IsDescendantOf(game) then
        local init_important = init_map:FindFirstChild("Important")
        if init_important and init_important.Parent and init_important:IsDescendantOf(game) then
            local init_baseplate_folder = init_important:FindFirstChild("Baseplate")
            if init_baseplate_folder and init_baseplate_folder.Parent and init_baseplate_folder:IsDescendantOf(game) then
                local init_base_part = init_baseplate_folder:FindFirstChild("Baseplate")
                if init_base_part and init_base_part.Parent and init_base_part:IsA("BasePart") then
                    init_base_part.Transparency = 1
                end
            end
        end
    else
        local room_extra = getgenv().Workspace:FindFirstChild("RoomExtra")
        if room_extra and room_extra.Parent and room_extra:IsDescendantOf(game) then
            for _, v in ipairs(room_extra:GetDescendants()) do
                if v and v.Parent and v:IsA("BasePart") and v.Name == "Baseplate" then
                    v.Transparency = 1
                end
            end
        end
...
elseif game.PlaceId == 80080558412215 then
    ...
    Flag = "MICUPTransparency",
    Callback = function(baseplate_transparency)
        if not getgenv().Baseplate_In_Current_Game_Flag then return end
        local parts = getgenv().found_the_modifiable_baseplate
        if not parts then return g.notify("Error", "Modifiable baseplate list not found.", 3) end
        for _, v in ipairs(parts) do
            if v and v.Parent and v:IsA("BasePart") then
                v.Transparency = baseplate_transparency
            end
        end
    end,})
else
...
Callback = function(Map_Color)
    for part, _ in pairs(getgenv().OriginalColors) do if part and part.Parent then part.Color = Map_Color end end
end,})

getgenv().Reset_Map_Color = Tab18:CreateButton({
Name = "Reset Map Colors",
Callback = function()
    for part, originalColor in pairs(getgenv().OriginalColors) do if part and part.Parent then part.Color = originalColor end end
end,})

getgenv().find_mic_up_baseplate = function()
    local map = getgenv().Workspace:FindFirstChild("map", true)
    if not map or not map.Parent or not map:IsDescendantOf(game) then return nil end
    local important = map:FindFirstChild("Important")
    if not important or not important.Parent or not important:IsDescendantOf(game) then return nil end
    task.wait(0.1)
    if not important or not important.Parent or not important:IsDescendantOf(game) then return nil end
    local baseplate_folder = important:FindFirstChild("Baseplate")
    if not baseplate_folder or not baseplate_folder.Parent or not baseplate_folder:IsDescendantOf(game) then return nil end
    local base_part = baseplate_folder:FindFirstChild("Baseplate")
    if not base_part or not base_part.Parent or not base_part:IsA("BasePart") then return nil end
    return base_part
end

getgenv().find_mic_up_baseplate_2 = function()
    local room_extra = getgenv().Workspace:FindFirstChild("RoomExtra")
    if not room_extra or not room_extra.Parent or not room_extra:IsA("Folder") or not room_extra:IsDescendantOf(game) then return nil end
    task.wait(0.1)
    if not room_extra or not room_extra.Parent or not room_extra:IsA("Folder") or not room_extra:IsDescendantOf(game) then return nil end
    for _, v in ipairs(room_extra:GetChildren()) do if v:IsA("BasePart") and v.Parent then return v end end
    return nil
end
...
getgenv().find_baseplate = function()
    for _, v in ipairs(getgenv().Workspace:GetDescendants()) do
        if v:IsA("BasePart") and string.find(v.Name:lower(), "baseplate") then
            ...
            if not getgenv().Baseplate_OriginalColors[v] then getgenv().Baseplate_OriginalColors[v] = v.Color end
            return v
        end
    end
    return nil
...
getgenv().apply_tween_color = function(part, new_color)
    local tweenInfo = TweenInfo.new(1,Enum.EasingStyle.Linear,Enum.EasingDirection.Out)
    local tween = getgenv().TweenService:Create(part,tweenInfo,{Color = new_color})
    tween:Play()
end
...
getgenv().update_baseplate_colors = function(baseplates)
    ...
    while getgenv().rainbow_universal_baseplate_ez == true do
        local new_color = colors[color_index]
        for _,part in ipairs(baseplates) do if part and part.Parent then apply_tween_color(part,new_color) end end
        color_index = (color_index % #colors) + 1
        task.wait(1)
    end
end
...
Callback = function(state)
    if state then
        ...
        if #baseplates == 0 then
            if getgenv().RainbowBaseplate_Universal then getgenv().RainbowBaseplate_Universal:Set(false) end
        ...
        for _,baseplate in ipairs(getgenv().Found_Baseplates) do
            if baseplate and baseplate.Parent then
                ...
                if original then baseplate.Color = original end
            end
        end
    end
end,})
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    ...
    Callback = function(Base_Color)
        ...
        if Mic_Up_Baseplate then Mic_Up_Baseplate.Color = Base_Color end
    end,})
    ...
    Callback = function(Size_New)
        ...
        if Baseplate_Local then Baseplate_Local.Size = Vector3.new(Size_New, Baseplate_Local.Size.Y, Baseplate_Local.Size.Z) end
    end,})
    ...
    Callback = function(Sized_Z)
        ...
        if Baseplate_Local_Z then Baseplate_Local.Size = Vector3.new(Baseplate_Local_Z.Size.X, Baseplate_Local_Z.Size.Y, Sized_Z) end
    end,})
elseif game.PlaceId == 136162036182779 then
    ...
    function change_AFK(boolean)
        if not Focus_RF or not Focus_RF.Parent or not Focus_RF:IsA("RemoteFunction") then
            getgenv().flashing_AFK_Title_Toggle = false
            return getgenv().notify("Error", "You cannot use this (did not find 'Focus' RemoteFunction) or Focus isn't a RemoteFunction.", 5)
        end
        if boolean == true then
            Focus_RF:InvokeServer(true)
        elseif boolean == false then
            Focus_RF:InvokeServer(false)
        end
    end
    wait(0.2)
    ...
    Callback = function(base_color)
        ...
        if not ws or not ws.Parent then return g.notify("Error", "Workspace not found.", 3) end
        local map = ws:FindFirstChild("map")
        if not map or not map.Parent then return g.notify("Error", "Map not found.", 3) end
        local lobby_folder = map:FindFirstChild("Lobby")
        if not lobby_folder or not lobby_folder.Parent then return g.notify("Error", "Lobby folder not found.", 3) end
        local base_part = lobby_folder:FindFirstChild("Baseplate")
        if not base_part or not base_part.Parent or not base_part:IsA("BasePart") then return g.notify("Error", "Baseplate not found.", 3) end
        base_part.Color = base_color
    end,})
    ...
    Increment = 1,
    Suffix = "",
    CurrentValue = getgenv().Flags and getgenv().Flags["ChangingBasePlateSized"] or 400,
    Flag = "ChangingBasePlateSized",
    Callback = function(size_new)
        local ws = getgenv().Workspace
        if not ws or not ws.Parent then return g.notify("Error", "Workspace not found.", 3) end
        ...
        local base_part = lobby_folder:FindFirstChild("Baseplate")
        if not base_part or not base_part.Parent or not base_part:IsA("BasePart") then return g.notify("Error", "Baseplate not found.", 3) end
        base_part.Size = Vector3.new(size_new, base_part.Size.Y, base_part.Size.Z)
    end,})
    ...
    Increment = 1,
    Suffix = "",
    CurrentValue = getgenv().Flags and getgenv().Flags["ChangingBaseSize"] or 400,
    Flag = "ChangingBaseSize",
    Callback = function(sized_z)
        local ws = getgenv().Workspace
        if not ws or not ws.Parent then return g.notify("Error", "Workspace not found.", 3) end
        ...
        local base_part = lobby_folder:FindFirstChild("Baseplate")
        if not base_part or not base_part.Parent or not base_part:IsA("BasePart") then return g.notify("Error", "Baseplate not found.", 3) end
        base_part.Size = Vector3.new(base_part.Size.X, base_part.Size.Y, sized_z)
    end,})

    getgenv().Flash_AFK_Title_Toggle = Tab2:CreateToggle({
    Name = "Flash AFK Title (FE, German Hangout)",
    CurrentValue = getgenv().flashing_AFK_Title_Toggle or false,
...
Callback = function()
    local Character = getgenv().Character or LocalPlayer.Character or g.get_char(LocalPlayer, 3)
    local the_player = get_whitelisted_plr_from_tbl()
    if not the_player then return getgenv().notify("Error", "Nobody found in whitelist table.", 5) end
    local Humanoid = getgenv().Character and getgenv().Humanoid or g.get_human(LocalPlayer, 3)
    local TheirCharacter = the_player and the_player.Character or g.get_char(the_player, 3)
    if not TheirCharacter or not TheirCharacter.Parent then return getgenv().notify("Error", tostring(the_player).." doesn't seem to have a Character (died?)", 6) end
    local get_their_humanoid = TheirCharacter and TheirCharacter:FindFirstChildWhichIsA("Humanoid") or g.get_human(the_player, 10)
    if the_player and TheirCharacter and TheirCharacter.Parent and TheirCharacter:IsDescendantOf(game) then
        for _, anim_track in pairs(get_their_humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end
    else
        pcall(function() for _, anim_track in pairs(Humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end end)
        wait(0.2)
...
Callback = function()
    if not getgenv().Character or not getgenv().Character.Parent then return g.notify("Error", "Character not found.", 3) end
    if not getgenv().Humanoid or not getgenv().Humanoid.Parent then return g.notify("Error", "Humanoid not found.", 3) end
    local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character and getgenv().Character:FindFirstChild("HumanoidRootPart") or g.get_root(LocalPlayer, 3)
    if not HumanoidRootPart or not HumanoidRootPart.Parent then return g.notify("Error", "Your HumanoidRootPart does not exist or was not found.", 3) end
    if getgenv().Humanoid.RigType == Enum.HumanoidRigType.R6 then return getgenv().notify("Error", "You have to be in R15 to use this!", 5) end
    local animate = getgenv().Character:FindFirstChild("Animate")
    if animate and animate.Parent and animate:IsA("LocalScript") and animate:IsDescendantOf(game) and animate.Disabled then animate.Disabled = false end
    wait(0.2)
    if not getgenv().Humanoid or not getgenv().Humanoid.Parent then return g.notify("Error", "Humanoid not found.", 3) end
    for _, anim_track in pairs(getgenv().Humanoid:GetPlayingAnimationTracks()) do anim_track:Stop() end
    wait(0.1)
    if not getgenv().Humanoid or not getgenv().Humanoid.Parent then return g.notify("Error", "Humanoid not found.", 3) end
    if getgenv().Humanoid.Sit then getgenv().Humanoid:ChangeState(3) end
    wait(0.2)
    if not HumanoidRootPart or not HumanoidRootPart.Parent then return g.notify("Error", "Your HumanoidRootPart does not exist or was not found.", 3) end
    if HumanoidRootPart.Anchored then
        if getgenv().FrozenChar then getgenv().FrozenChar:Set(false) end
        HumanoidRootPart.Anchored = false
        wait(0.2)
        if not getgenv().Humanoid or not getgenv().Humanoid.Parent then return g.notify("Error", "Humanoid not found.", 3) end
...
RemoveTextAfterFocusLost = true,
Callback = function(input_id)
    if not getgenv().Character or not getgenv().Character.Parent then return g.notify("Error", "Character not found.", 3) end
    if not getgenv().Humanoid or not getgenv().Humanoid.Parent then return g.notify("Error", "Humanoid not found.", 3) end
    local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character and getgenv().Character:FindFirstChild("HumanoidRootPart") or g.get_root(LocalPlayer, 3)
    if not HumanoidRootPart or not HumanoidRootPart.Parent then return g.notify("Error", "Your HumanoidRootPart does not exist or was not found.", 3) end
    if getgenv().Humanoid.RigType == Enum.HumanoidRigType.R6 then return getgenv().notify("Error", "You have to be in R15 to use this!", 5) end
    local num_id = tonumber(input_id) or input_id
    if not num_id then return end
    local succ, err = pcall(function()
        getgenv().Humanoid:PlayEmoteAndGetanim_trackById(num_id)
    end)
    if not succ then return getgenv().notify("Error", tostring(err), 7) end
end,})
print("32")
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    ...
    CurrentValue = getgenv().ava_switch or false,
    Flag = "quickAvChange",
    Callback = function(quickAv)
        local RS = getgenv().ReplicatedStorage
        if not RS or not RS.Parent then return g.notify("Error", "ReplicatedStorage not found.", 3) end
        local modify_remote = RS:FindFirstChild("ModifyUsername")
        if not modify_remote or not modify_remote.Parent or not modify_remote:IsA("RemoteEvent") then return g.notify("Error", "ModifyUsername remote not found.", 3) end
        if quickAv then
            local players = getgenv().Players
            if not players then return g.notify("Error", "Players service not found.", 3) end
            ...
            while getgenv().ava_switch == true do
                task.wait()
                if not modify_remote or not modify_remote.Parent or not modify_remote:IsA("RemoteEvent") then break end
                for _, username in ipairs(usernames) do
                    if not getgenv().ava_switch then break end
                    if not modify_remote or not modify_remote.Parent or not modify_remote:IsA("RemoteEvent") then break end
                    modify_remote:FireServer(username)
                end
            end
        else
            ...
            task.wait(0.3)
            local lp = getgenv().LocalPlayer
            if not lp then return g.notify("Error", "LocalPlayer not found.", 3) end
            if not modify_remote or not modify_remote.Parent or not modify_remote:IsA("RemoteEvent") then return g.notify("Error", "ModifyUsername remote not found.", 3) end
            modify_remote:FireServer(tostring(lp.Name))
        end
    end,})
else
...
Callback = function(face_Bang_Animation)
    ...
    if enabled then
        ...
        getgenv().heartFaceFckConn = getgenv().UserInputService.InputBegan:Connect(function(input, game_processed)
            if not game_processed and input.KeyCode == fKey then
                fuck()
            end
        end)
        task.wait()
...
Callback = function()
    if getgenv().flashbacks_script then
        return getgenv().notify("Error", "Flashback Rewind is already running!", 5)
    end

    local key = Enum.KeyCode[getgenv().settings_flashback.Keybind]
    if not Enum.KeyCode[getgenv().settings_flashback.Keybind] then
        return getgenv().notify("Warning", "KeyCode seems to be invalid, try another one", 5)
    end
    ...
    getgenv().on_render_step = function()
        if getgenv().Character:FindFirstChild("HumanoidRootPart") and getgenv().Humanoid then
            local character = getgenv().Character
            local HumanoidRootPart = getgenv().HumanoidRootPart or getgenv().Character:FindFirstChild("HumanoidRootPart")
            local humanoid = getgenv().Humanoid
    ...
    if character and humanoid and HumanoidRootPart then RunService:BindToRenderStep("FlashbackStep", 1, on_render_step) end
    wait(.2)
...
Callback = function()
    ...
    if getgenv().flashbacks_script then
        ...
        return getgenv().notify("Warning", "Flashback Rewind has not been loaded.", 3)
    end
end,})

if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    getgenv().hoverboard_fly_speed = getgenv().hoverboard_fly_speed or 10
    getgenv().hoverboard_flying = getgenv().hoverboard_flying or false
    getgenv().InfoLabelParagraph = Tab13:CreateParagraph({Title = "Information:", Content = "Toggle to start/stop hoverboard flying."})
    if not getgenv().UserInputService.TouchEnabled then getgenv().InfoLabelParagraph_2 = Tab13:CreateParagraph({Title = "Controls:", Content = "WASD Move | Space Up | Q Down"}) end
    ...
    getgenv().find_cloud_tool_for_hoverboard_fly = function()
        for _, v in ipairs(getgenv().LocalPlayer:FindFirstChildWhichIsA("Backpack"):GetChildren()) do
    ...
    Callback = function(speed_value)
        if not speed_value then return g.notify("Warning", "Please input a number.", 3) end
    ...
    Callback = function(toggle_hoverboard_fly)
        ...
        if not cached or not cached:IsA("RemoteEvent") then
            if getgenv().hoverboard_fly_toggle then getgenv().hoverboard_fly_toggle:Set(false) end
            return getgenv().notify("Error", "Tool giver RemotEvent not found or is not a RemoteEvent.", 5)
        end
        ...
        if toggle_hoverboard_fly then
            ...
            local character = getgenv().Character or getgenv().LocalPlayer.Character or g.get_char(LocalPlayer, 5)
            local root_part = getgenv().HumanoidRootPart or character and character:FindFirstChild("HumanoidRootPart") or g.get_root(LocalPlayer, 5)
            local humanoid = getgenv().Humanoid or character and character:FindFirstChildWhichIsA("Humanoid") or g.get_human(LocalPlayer, 5)
            ...
            getgenv().FlamesLibrary.connect("hoverboard_fly_loop", getgenv().RunService.RenderStepped:Connect(function()
                if not getgenv().hoverboard_flying then return end
                ...
                if input:IsKeyDown(Enum.KeyCode.W) then move_direction += look_vector end
                if input:IsKeyDown(Enum.KeyCode.S) then move_direction -= look_vector end
                if input:IsKeyDown(Enum.KeyCode.A) then move_direction -= right_vector end
                if input:IsKeyDown(Enum.KeyCode.D) then move_direction += right_vector end
                if input:IsKeyDown(Enum.KeyCode.Space) then move_direction += Vector3.new(0,1,0) end
                if input:IsKeyDown(Enum.KeyCode.Q) then move_direction -= Vector3.new(0,1,0) end
            ...
            local humanoid = getgenv().Humanoid or character and character:FindFirstChildWhichIsA("Humanoid") or g.get_human(player, 5)
            if hover and hover:IsA("Tool") then
                local hover = character.Hoverboard
                hover.Parent = g.Backpack or player.Backpack
                task.wait(0.2)
                if getgenv().found_tool_deleter_Remote_Event and getgenv().found_tool_deleter_Remote_Event:IsA("RemoteEvent") then getgenv().found_tool_deleter_Remote_Event:FireServer() end
            end

            if root_part and root_part.Parent and root_part:IsDescendantOf(game) then
                if root_part:FindFirstChildOfClass("BodyGyro") then root_part:FindFirstChildOfClass("BodyGyro"):Destroy() end
                if root_part:FindFirstChildOfClass("BodyVelocity") then root_part:FindFirstChildOfClass("BodyVelocity"):Destroy() end
            end
...
if g.fps_cap and typeof(g.fps_cap) == "function" then
    ...
    Callback = function(fps_to_set)
        local fps_cap = g.set_fps
        if fps_cap and typeof(fps_cap) == "function" then g.fps_cap(fps_to_set) end
    end,})
elseif identifyexecutor and typeof(identifyexecutor) == "function" then
    getgenv().notify("Error", "Your executor: "..tostring(executor_Name).." does not support 'setfpscap'", 5)
else
    getgenv().notify("Error", "Your executor does is garbage.", 5)
end
...
if getgenv().LocalPlayer:FindFirstChildOfClass("AudioDeviceInput") then
    ...
    Callback = function(muteYou)
        if muteYou then
            ...
            if not input then
                ...
                return getgenv().notify("Error", "AudioDeviceInput does not exist in LocalPlayer!", 5)
            end
    ...
    if Enum.KeyCode[keybindString] then
        ...
        getgenv().notify("Error", "Invalid keybind string: "..tostring(keybindString), 5)
    ...
    if getgenv().already_loaded_mute_keybind then
        ...
        getgenv().UserInputService.InputBegan:Connect(function(input, game_processed)
            if game_processed then return end
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    ...
    Callback = function()
        ...
        if not Delete_Inv_RemoteEvent then return getgenv().notify("Error", "ToolDelete RemoteEvent doesn't exist in ReplicatedStorage!", 5) end
...
if hookfunction and hookmetamethod then
    ...
    Callback = function()
        ...
        if TextChatService.ChatVersion == Enum.ChatVersion.TextChatService then
            getgenv().notify("Error", "Detected new chat, loading Anti Screenshot...", 5)
            if setfflag then
                pcall(function()
        ...
        if not ChatMain then
            ...
            if not ChatMain then
                getgenv().notify("Error", "Did not find ChatMain, loading Anti Screenshot instead...", 5)
                if setfflag then
                    pcall(function()
                        setfflag("AbuseReportScreenshot", "False")
                        setfflag("AbuseReportScreenshotPercentage", "0")
        ...
        if not PostMessage then
            getgenv().notify("Error", "PostMessage could not be located, loading Anti Screenshot...", 5)
            if setfflag then
                pcall(function()
                    setfflag("AbuseReportScreenshot", "False")
                    setfflag("AbuseReportScreenshotPercentage", "0")
...
Callback = function()
    if getgenv().FakeChat_Loaded then
        return getgenv().notify("Error", "Fake Chat GUI has already been loaded!", 5)
...
if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
    ...
    Callback = function()
        ...
        if not Prison_Life_Map then
            return getgenv().notify("Error", "Did not find allocated Prison_Life Map.", 5)
        end
        local Prison_Spawn = Prison_Life_Map:FindFirstChild("Prison_spawn")
        if not Prison_Spawn then
            return getgenv().notify("Error", "Did not find allocated Prison_Spawn Part.", 5)
        end
        local Nexus_Prison_Spawn = Prison_Spawn:FindFirstChild("Nexus")
        if not Nexus_Prison_Spawn then
            return getgenv().notify("Error", "Did not find allocated Nexus Script Spawn.", 5)
        end
        wait()
...
Callback = function(speedCoil)
    if speedCoil then
        ...
        local function setupCharacter(character)
            ...
            local function onShiftPress(input, game_processed)
                if input.KeyCode == Enum.KeyCode.LeftShift and not game_processed and getgenv().runningEnabled then
                    humanoid.WalkSpeed = getgenv().run_Shift_Speed
                end
            end

            local function onShiftRelease(input, game_processed)
                if input.KeyCode == Enum.KeyCode.LeftShift and not game_processed and getgenv().runningEnabled then
...
create_Button(zombieAnim, "Zombie Animation Package", function()
    ...
    wait()
    Animate.Disabled = false
    local anim_track = getgenv().Character:FindFirstChildOfClass("Humanoid"):GetPlayingAnimationTracks()
    for i, track in pairs (anim_track) do
        track:Stop()
    end
    wait()
...
create_Button(HeroAnim, "Hero Animation Package", function()
    ...
    wait()
    Animate.Disabled = false
    local anim_track = getgenv().LocalPlayer.Character:FindFirstChildOfClass("Humanoid"):GetPlayingAnimationTracks()
    for i, track in pairs (anim_track) do
        track:Stop()
    end
    wait()
...
create_Button(ghostAnim, "(FE) Ghost Animation Package", function()
    ...
    wait()
    Animate.Disabled = false
    local anim_track = getgenv().Humanoid:GetPlayingAnimationTracks()
    for i, track in pairs (anim_track) do
        track:Stop()
    end
    wait()
...
Callback = function(isTripOn)
    if isTripOn then
        ...
        getgenv().Trip_Connection = UserInputService.InputBegan:Connect(function(input, game_processed)
            if game_processed then return end
...
wait()
if getgenv().Trip_Enabled == true then getgenv().Trip_Enabled = false getgenv().Trip_Script:Set(false) end
wait()
function del_ez_Config()
    if not isfolder or typeof(isfolder) ~= "function" then return end
    if not delfolder or typeof(delfolder) ~= "function" then return end
    if isfolder("executor_configs/") then
        delfolder("executor_configs/")
        getgenv().notify("Success", "Configuration Folder has been deleted.", 3)
    else
        getgenv().notify("Error", "No saved Configuration Folder to delete.", 3)
    end
end
...
function load_ez_Config()
    ...
    for _, key in ipairs(config_keys) do
        ...
        if isfile(filePath) then
            ...
            local decoded_data = getgenv().HttpService:JSONDecode(file_content)
            config_table[key] = decoded_data[key]
        else
            config_table[key] = "off"
        end
    end
end

function read_config_value(key)
...
function get_animation(animationType)
    ...
    if isfile(filePath) then
        ...
        return decoded_data[animationType]
    else
        return "Zombie"
    end
end

getgenv().DeleteYourCustomConfiguration = Tab20:CreateButton({
...
if isfolder and makefolder and typeof(isfolder) == "function" and typeof(makefolder) == "function" then
    if not isfolder(ez_folder_settings) then makefolder(ez_folder_settings) end
end
getgenv().fake_out = function()
    wait(0.5)
    OrgDestroyHeight = workspace.FallenPartsDestroyHeight
    wait(0.1)
    local root = getgenv().HumanoidRootPart or getgenv().Character:FindFirstChild("HumanoidRootPart") or g.get_root(g.LocalPlayer, 5)
...
Callback = function(isFakeOutEnabled)
    if isFakeOutEnabled then
        ...
        getgenv().FakeOut_Connection = UserInputService.InputBegan:Connect(function(input, game_processed)
            if game_processed then return end
...
if not getgenv().camera_zoom_data then
    pcall(function() getgenv().LocalPlayer.CameraMaxZoomDistance = 800000 end)
    getgenv().camera_zoom_data = true
end
wait(0.25)
if getgenv().SimpleSpyExecuted then if getgenv().SimpleSpyShutdown and typeof(getgenv().SimpleSpyShutdown) == "function" then getgenv().SimpleSpyShutdown() end end
wait(0.1)
TerrainFolder = getgenv().Workspace:FindFirstChild("TERRAIN_EDITOR") or Instance.new("Folder", Workspace)
TerrainFolder.Name = "TERRAIN_EDITOR"
Transparency_Selected = 1
task.wait(1)
if TerrainFolder and TerrainFolder.Parent and TerrainFolder:IsA("Folder") then
...
if not getgenv().output_already_viewed then
    if game.PlaceId == 6884319169 or game.PlaceId == 15546218972 then
        ...
        if not getgenv().Has_Died_Func then
            ...
            getgenv().is_Flames_Attached = true
...
if not getgenv().seen_output then
    if getgenv().is_Flames_Attached then
        pcall(function()
            GuiService:SendNotification({
                Title = "Successful.",
                Text = "Successfully injected into the current experience."
            })
        end)
    else
        pcall(function()
